...
func main() {
	...
	app := &cli.App{
		...
		Commands: []*cli.Command{
			...
			sealBenchCmd,
			importBenchCmd,
			generateWinningPostCmd,
			verifyWinningPostCmd,
			generateWindowPostCmd,
			verifyWindowPostCmd,
			//windowCmd,
			sealCmd,
		},
...
func runSeals(sb *ffiwrapper.Sealer, sbfs *basicfs.Provider, numSectors int, par ParCfg, mid abi.ActorID, sectorSize abi.SectorSize, ticketPreimage []byte, saveC2inp string, skipc2, skipunseal bool) ([]SealingResult, []saproof.SectorInfo, error) {
	...
	for i := abi.SectorNumber(1); i <= abi.SectorNumber(numSectors); i++ {
		...
		//fmt.Printf("add piece: pieceInfo is {Size: %d, PieceCID/commD: %s\n}", pi.Size, pi.PieceCID)
		fmt.Println("add piece")
		fmt.Printf("sectorID is %+v\n", sid)
		fmt.Printf("pieceInfo is %+v\n}", pi)
		...
		go func(worker int) {
			sealerr := func() error {
				...
				for i := abi.SectorNumber(start); i < abi.SectorNumber(end); i++ {
					...
					fmt.Printf("preCommit2: cids is {Sealed: %s, Unsealed: %s}\n", cids.Sealed, cids.Unsealed)
					...
					fmt.Printf("commit1: len of c1o is %d", len(c1o))

					sealcommit1 := time.Now()

					log.Infof("[%d] Generating PoRep for sector (2)\n", i)
					...
					fmt.Printf("commit2: len of PoRep proof is %d, PoRep proof is %x\n", len(proof), proof)
...
func runSealsModify(sb *ffiwrapper.Sealer, sbfs *basicfs.Provider, numSectors int, par ParCfg, mid abi.ActorID, sectorSize abi.SectorSize, ticketPreimage []byte, saveC2inp string, skipc2, skipunseal bool, filepath string) ([]SealingResult, []saproof.SectorInfo, error) {
	var pieces []abi.PieceInfo
	sealTimings := make([]SealingResult, numSectors)
	sealedSectors := make([]saproof.SectorInfo, numSectors)
	...
	for i := abi.SectorNumber(1); i <= abi.SectorNumber(numSectors); i++ {
		...
		pi, err := sb.AddPiece(context.TODO(), sid, nil, abi.PaddedPieceSize(sectorSize).Unpadded(), storage.Data(reader))
		//pi, err := sb.AddPiece(context.TODO(), sid, nil, abi.PaddedPieceSize(fileInfo.Size()).Unpadded(), storage.Data(reader))
