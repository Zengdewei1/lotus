...
func (sb *Sealer) AddPiece(ctx context.Context, sector abi.SectorID, existingPieceSizes []abi.UnpaddedPieceSize, pieceSize abi.UnpaddedPieceSize, file storage.Data) (abi.PieceInfo, error) {
	...
	if len(existingPieceSizes) == 0 {
		...
		fmt.Printf("stagedPath is %s\n", stagedPath)
		fmt.Printf("sector is %+v\n", sector)
...
//func (sb *Sealer) AddPieceModify(ctx context.Context, sector abi.SectorID, existingPieceSizes []abi.UnpaddedPieceSize, pieceSize abi.UnpaddedPieceSize, file storage.Data) (abi.PieceInfo, error) {
//	var offset abi.UnpaddedPieceSize
//	for _, size := range existingPieceSizes {
//		offset += size
//	}
//
//	maxPieceSize := abi.PaddedPieceSize(sb.ssize)
//	fmt.Printf("maxPieceSize is %d", maxPieceSize)
//
//	if offset.Padded()+pieceSize.Padded() > maxPieceSize {
//		return abi.PieceInfo{}, xerrors.Errorf("can't add %d byte piece to sector %v with %d bytes of existing pieces", pieceSize, sector, offset)
//	}
...
//	var stagedPath stores.SectorPaths
//	if len(existingPieceSizes) == 0 {
//		stagedPath, done, err = sb.sectors.AcquireSector(ctx, sector, 0, )
//		if err != nil {
//			return abi.PieceInfo{}, xerrors.Errorf()
//		}
//
//		stagedFile, err = createPartialFile(maxPieceSize, stagedPath.Unsealed)
//		if err != nil {
